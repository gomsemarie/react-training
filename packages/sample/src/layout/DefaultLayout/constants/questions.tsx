...
export const TRAINING_QUESTIONS: TrainingType[] = [
  ...
  },
  {
    ...
    desc: [
      `[ko]`,
      `- 어렵지만 React의 정수가 담긴 예제입니다. 이것만 마스터 하면 당신도 React 중수!`,
      `- props에는 Dropdown의 값 목록인 list, 기본값인 defaultValue, onChange 함수가 있습니다.`,
      `- <ul> 태그 내부에 <li> 태그를 사용해 Dropdown list를 출력해 주세요. (printList 함수 내부를 구현해 주시면 됩니다)`,
      ...
      ``,
      `[en]`,
      `- It's hard, but it's an example that contains the essence of React. If you master this, you will become a React Intermediate!`,
      `- The props include a list of Dropdown values, a default value of defaultValue, and an onChange function.`,
      `- Please use the <li> tag inside the <ul> tag to display the dropdown list. (You can implement it inside the printList function)`,
